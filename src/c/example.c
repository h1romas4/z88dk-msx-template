 ...
 */
void print_state() {
    char score_string[VRAM_WIDTH + 1];
    sprintf(score_string, "SCORE %06u  HISCORE %06u  %02u", game.score,
            game.score_hi, game.remein_clear);
...
void game_main(uint8_t stick) {
    ...
    if (ball.x > 1 && game.stick_state & st_left)
        ball_vx = -1;
    if (ball.x < 30 && game.stick_state & st_right)
        ball_vx += 1;
